...
6. Combined script :
   ...
   You can use the following script to trigger all the above commands:.
   ```
   python3 combined_script.py -fwver 0.0.2.1 -lfwver 0.0.0.0 -config config.json -p Certificates/QcFMPCert.pem -x Certificates/QcFMPRoot.pub.pem -oc Certificates/QcFMPSub.pub.pem -guid 6F25BFD2-A165-468B-980F-AC51A0A45C52 -capsule combined_kodiak1.cap -images /Images -setup
   ```
